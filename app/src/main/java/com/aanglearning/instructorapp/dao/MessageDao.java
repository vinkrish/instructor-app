...
public class MessageDao {
    ...
    public static List<Message> getGroupMessages(long groupId) {
        List<Message> messages = new ArrayList<>();
        SQLiteDatabase sqliteDatabase = AppGlobal.getSqlDbHelper().getReadableDatabase();
        String query = "select A.*, B.TeacherName from message A, teacher B where GroupId=" + groupId + " and A.SenderId=B.Id order by A.Id desc limit 100";
        Cursor c = sqliteDatabase.rawQuery(query, null);
        c.moveToFirst();
        while (!c.isAfterLast()) {
            Message message = new Message();
    ...
    public static List<Message> getGroupMessagesFromId(long gropId, long messageId) {
        List<Message> messages = new ArrayList<>();
        SQLiteDatabase sqliteDatabase = AppGlobal.getSqlDbHelper().getReadableDatabase();
        String query = "select A.*, B.TeacherName from message A, teacher B where GroupId=" + gropId + " and A.SenderId=B.Id and A.Id<" + messageId + " order by A.Id desc limit 100";
        Cursor c = sqliteDatabase.rawQuery(query, null);
        c.moveToFirst();
        while (!c.isAfterLast()) {
            Message message = new Message();
